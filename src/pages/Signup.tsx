...
export default function Signup() {
  ...
  const handleGuestSignup = async () => {
    ...
    try {
      ...
      console.log("Guest signup successful:", result.user.uid);
      navigate('/dashboard');
    } catch (err: any) {
      console.error("Guest signup failed:", err);
      setError(`Guest signup failed: ${err.message}. Please ensure Anonymous Authentication is enabled in the Firebase Console.`);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-8 font-body">
      <div className="w-full max-w-2xl border border-line bg-surface p-12 lg:p-20">
        <div className="flex flex-col items-center mb-16">
          ...
          <p className="text-2xl font-heading italic tracking-tight text-text-muted mb-6">Join our community</p>
          <div className="flex items-center gap-3">
            <span className="col-header">Create your account</span>
        ...
        <div className="mt-16 pt-12 border-t border-line text-center">
          <p className="font-mono text-[10px] uppercase tracking-widest text-text-muted">
            Already have an account? <Link to="/login" className="text-primary font-bold ml-2 hover:underline">Sign in</Link>
          </p>
        </div>
      </div>
    </div>
